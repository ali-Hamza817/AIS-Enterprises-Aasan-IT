...
import { PagePrivacy, PageDeleteAccount } from './legal'

// ── Navigation ────────────────────────────────────────────────────
export type Page = 'home' | 'apps' | 'about' | 'contact' | 'privacy' | 'delete-account'

// Each page has a real URL so it can be linked directly (e.g. from Google Play).
...
const PATH_ALIASES: Record<string, Page> = {
  '/apps': 'apps',
  '/privacy': 'privacy',
  '/privacy_policy': 'privacy',
  '/asan-dairy/privacy-policy': 'privacy',
...
function pageFromPath(pathname: string): Page {
  ...
  const match = (Object.keys(PAGE_PATHS) as Page[]).find((p) => PAGE_PATHS[p] === clean)
  return match ?? PATH_ALIASES[clean] ?? 'home'
}
    ...
    <footer style={{ background: 'var(--footer-bg)', color: '#94a3b8' }}>
          ...
          <div>
            ...
            {([
              ...
              <a
                ...
                style={{
                  display: 'block', color: '#64748b', fontSize: '0.875rem', padding: '0.3rem 0',
                  fontFamily: 'Inter, sans-serif', textDecoration: 'none', transition: 'color 0.2s',
                }}
                onMouseEnter={(e) => { e.currentTarget.style.color = '#93c5fd' }}
                onMouseLeave={(e) => { e.currentTarget.style.color = '#64748b' }}
              >
      ...
      <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
        <div className="max-w-7xl mx-auto px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1" style={{ fontSize: '0.82rem' }}>
            {([['Privacy Policy', 'privacy'], ['Delete Account', 'delete-account']] as [string, Page][]).map(([label, pg]) => (
              <a
                key={pg}
                href={PAGE_PATHS[pg]}
                onClick={(e) => { e.preventDefault(); navigate(pg) }}
            ...
            <span style={{ color: '#475569' }}>Dream • Create • Innovate</span>
          </div>
        </div>
      </div>
    </footer>
...
export default function App() {
  ...
  // Keep the URL and browser back/forward buttons in sync with the page.
  useEffect(() => {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--background)', color: 'var(--foreground)' }}>
      ...
      <main style={{ flex: 1 }}>
        <div key={page} className="tab-pane">
          ...
          {page === 'privacy' && <PagePrivacy setPage={setPage} />}
          {page === 'delete-account' && <PageDeleteAccount setPage={setPage} />}
        </div>
      </main>
